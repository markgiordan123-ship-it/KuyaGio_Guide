...
app.run_polling()        "Mega Fortune","Wild Spin","Cash Mania","Gold Rush","Temple Run","Mystic Gems",
        "Fortune Spin","Dragon Rise"
    ],
    "PG": [
        "Mahjong Ways","Mahjong Ways 2","Lucky Neko","Fortune Tiger","Dragon Hatch","Wild Bandito",
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("page_"):
        ...
        kb = [
            [
                InlineKeyboardButton("⬅️ BACK", callback_data=f"page_{provider}_{page-1 if page>0 else 0}"),
                InlineKeyboardButton("🏠 MENU", callback_data="menu"),
                InlineKeyboardButton("➡️ NEXT", callback_data=f"page_{provider}_{page+1 if page<4 else 4}")
            ]
        ]
