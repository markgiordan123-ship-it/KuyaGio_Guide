...
def is_spam(user_id):
    now = time.time()
    cur.execute("SELECT last_time FROM user_cooldown WHERE user_id=?", (user_id,))
    row = cur.fetchone()
    ...
    cur.execute("REPLACE INTO user_cooldown (user_id, last_time) VALUES (?,?)", (user_id, now))
    conn.commit()
    return False

# ---------------- GAME DATA (SCALED) ----------------
def expand(lst):
    return lst * 5  # auto scale to 50-ish entries

games = {
"JILI": expand(["SUPER ACE","GOLDEN EMPIRE","BOXING KING","CRAZY777","MONEY COMING","LUCKY JAGUAR","FORTUNE GEMS","WILD ACE","GOLDEN BANK 2","SHANGHAI BEAUTY"]),
"PG": expand(["Mahjong Ways","Lucky Neko","Fortune Tiger","Dragon Hatch","Ganesha Gold","Wild Bandito","Treasures of Aztec","Medusa","Rise of Apollo","Flirting Scholar"]),
"PRAGMATIC": expand(["Sweet Bonanza","Gates of Olympus","Starlight Princess","Sugar Rush","Big Bass Bonanza","Wolf Gold","Buffalo King","Fruit Party","Madame Destiny","Hot Fiesta"]),
"FA CHAI": expand(["Golden Monkey","Lucky Twins","Dragon Treasure","Fortune Festival","God of Wealth","Golden Panda","Lucky Koi","Five Dragons","Money Tree","Red Envelope"]),
"BNG": expand(["Bonanza Gold","Book of Fortune","Super Marble","Legendary Monkey","Shark Hunter","Dragon Fishing","Ocean King","Lucky Wheel BNG","Fire Rooster","Treasure Island"]),
"JDB": expand(["Dragon Hunter","Fire Phoenix","Money Bang Bang","Golden Disco","Candy Burst JDB","Lucky Star","Super Dragon","Hot Spin","Fortune Island","Crazy Money"]),
"YELLOW BAT": expand(["Yellow Bat Riches","Bat Frenzy","Golden Night Bat","Shadow Bat","Vampire Bat Gold","Lucky Bat Empire","Bat Treasure","Midnight Bat","Bat King","Neon Bat Rush"]),
"CO9": expand(["CO9 Fortune","Golden Empire CO9","Lucky Spin CO9","Dragon Rise CO9","Money Train CO9","Jungle King CO9","Mega Win CO9","Fire Wheel CO9","Treasure Box CO9","Wild Gold CO9"])
}
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("prov_"):
        ...
        page_items, page, maxp = paginate(data_map, 0)
    ...
    if data.startswith("page_"):
        ...
        data_map = build(provider)
        page_items, page, maxp = paginate(data_map, page)
...
app.run_polling()"FA CHAI": ["Golden Monkey","Lucky Twins","Dragon Treasure","Fortune Festival","God of Wealth","Golden Panda","Lucky Koi","Five Dragons","Money Tree","Red Envelope"] * 5,
