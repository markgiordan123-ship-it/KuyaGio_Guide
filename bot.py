...
# Pagination settings
PAGE_SIZE = 5  # Games per page

# Anti-spam cooldown (user_id -> timestamp)
cooldown = {}

def anti_spam(uid: int, cooldown_seconds: float = 1.5) -> bool:
...
# Game data - clean version
games = {
    "JILI": [
        "Super Ace", "Golden Empire", "Boxing King", "Crazy 777", "Money Coming",
        "Lucky Jaguar", "Fortune Gems", "Wild Ace", "Golden Bank 2", "Shogun",
        "3 Lucky Pigs", "3 Coin Treasures", "Nightfall Hunting", "Money Pot",
        "Fruity Wheel", "Aztec Priestess", "Bangla Beauty", "Go For Champion",
        "Egypt Glow", "Magic Lamp", "Night City", "Legacy of Egypt", "Pirate Queen",
        "Golden Temple", "Jackpot Joker", "Candy Baby", "Mines Gold",
        "Lucky Goldbricks", "Bonus Hunter", "Party Star", "King Arthur",
        "War Dragons", "Book of Gold", "Sweet Land", "Boxing Extravaganza",
        "Sin City", "Golden Bank", "Pharaoh Treasure", "Witches Night",
        "Arena Fighter", "Lucky Doggy", "Fortune Tree", "Bone Fortune",
        "Golden Queen", "Master Tiger", "Jungle King", "Samba", "Golden Joker"
    ],
    "PG": [
        "Mahjong Ways 1", "Mahjong Ways 2", "Lucky Neko", "Fortune Tiger",
        "Dragon Hatch", "Wild Bandito", "Treasures of Aztec", "Ganesha Gold",
        "Medusa", "Symbol of Egypt", "Hood vs Wolf", "Rooster Rumble",
        "Win Win Fish", "Garuda Gems", "Bikini Paradise", "Double Fortune",
        "Crypto Gold", "Dragon Legend", "Candy Burst", "Phoenix Rises",
        ...
        "Shark Hunter", "Bali Vacation", "Piggy Gold", "Opera Dynasty",
        "Wild Fireworks", "Legend Perseus", "Leprechaun Riches", "Crypto Panda",
        "Buffalo Win", "Mahjong Ways 3", "Golden Pig", "Lucky Clover",
        "Supermarket Spree", "Mahjong Royal", "Fortune Rabbit", "Candy Bonanza"
    ],
    "PRAGMATIC": [
        "Gates of Olympus", "Sweet Bonanza", "Sugar Rush", "Big Bass Bonanza",
        "Wolf Gold", "The Dog House", "Wild West Gold", "Buffalo King",
        "Madame Destiny", "Fire Strike", "Aztec Gems", "John Hunter",
        "Release Kraken", "Hot Safari", "Extra Juicy", "Fruit Party",
        "Sugar Rush 1000", "Sweet Xmas", "Olympus 1000", "Starlight Princess",
        "Power Thor", "Viking Forge", "Hand Midas", "Caesars Gold", "Chilli Heat",
        "5 Lions Megaways", "Mustang Gold", "Madame Megaways", "Bronco Spirit",
        "Cowboy Coins", "Pixie Wings", "Aztec King", "Wild Walker", "Cosmic Cash",
        "Treasure Wild", "Fruit Party 2", "Gates Hades", "Powernudge",
        "Bigger Bass", "Black Bull", "Gold Party", "Fire Hot 40",
        "Lucky Lightning", "Magic Maze", "Super X", "Vegas Nights",
        "Ultra Hold", "Mystery Symbols", "Golden Odyssey"
    ],
    # Add other providers as needed...
}

def get_time() -> str:
    """Get time window for recommendations"""
    now = datetime.now(PH)
...
def paginate_keyboard(provider: str, page: int) -> InlineKeyboardMarkup:
    """Create pagination keyboard"""
    items = games[provider]
    max_page = (len(items) - 1) // PAGE_SIZE
    
    buttons = [
        [
            InlineKeyboardButton("⬅️ Prev", callback_data=f"page_{provider}_{page-1}" if page > 0 else "noop"),
            InlineKeyboardButton(f"{page + 1}/{max_page + 1}", callback_data="noop"),
            InlineKeyboardButton("Next ➡️", callback_data=f"page_{provider}_{page+1}" if page < max_page else "noop")
        ],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        f"🎮 Welcome to KuyaGio Guide!\n\n📖 Full Guide: {CANVA_LINK}\n\nChoose a provider below 👇",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Anti-spam check
    if anti_spam(query.from_user.id):
    ...
    if data.startswith("page_"):
        parts = data.split("_")
        provider = "_".join(parts[1:-1])  # Handle providers with underscores
        page = int(parts[-1])
        ...
        await query.edit_message_text(msg, reply_markup=paginate_keyboard(provider, page), parse_mode="Markdown")
        return
    ...
    if data in games:
        ...
        await query.edit_message_text(msg, reply_markup=paginate_keyboard(data, 0), parse_mode="Markdown")
...
if __name__ == "__main__":
    ...
    print("🤖 Bot started...")
    app.run_polling()e Destiny","Fire Strike",
...
"Mega Circus","Dragon Gold Spin","Wild Boom","Hot Candy","Fire Fortune",
...
# ---------------- FIXED GAME DATA (CLEANED NAMES ONLY) ----------------
games = {
"JILI": [
"Super Ace","Golden Empire","Boxing King","Crazy 777","Money Coming","Lucky Jaguar",
"Fortune Gems","Wild Ace","Golden Bank 2","Shogun","3 Lucky Pigs","3 Coin Treasures",
"Nightfall Hunting","Money Pot","Fruity Wheel","Aztec Priestess","Bangla Beauty",
"Go For Champion","Egypt Glow","Magic Lamp","Night City","Legacy of Egypt",
"Pirate Queen","Golden Temple","Jackpot Joker","Candy Baby","Mines Gold",
